...
function getImageSize3(buffer) {
  const metadata = imageSize(buffer);
  return { width: metadata.width || 0, height: metadata.height || 0 };
}
__name(getImageSize3, "getImageSize");
var index_default = {
  async fetch(request, env, ctx) {
    ...
    const root = (0, import_node_html_parser.parse)(answerText, {
    ...
    allText = allText.replace(/<[^>]+>/g, "").trim().toLowerCase();
    return new Response(
      JSON.stringify({
